...
int main(){
    ODEVectors vectors;
    vectors.sizeOfSystem = 55;
    vectors.numOfSystems = 1 << 3;

    mechanism_memory *h_mem = (mechanism_memory*)malloc(sizeof(mechanism_memory));
    mechanism_memory *d_mem = nullptr;

    initialize_gpu_memory(vectors.numOfSystems, &h_mem, &d_mem);
    ...
    vectors.var = (scalar*)malloc(vectors.numOfSystems * sizeof(scalar));

    auto start_alloc = std::chrono::high_resolution_clock::now();

    set_same_initial_conditions(vectors.numOfSystems, vectors.data, vectors.var);
...
__global__
void seulex_solve(kodes::GRIMESHSystem* ode, scalar* data, label numOfSystems, stepState step, scalar xEnd, scalar* resouces_scalar, label* resouces_label)
{
