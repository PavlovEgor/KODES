...
namespace Foam
{
    defineTypeNameAndDebug(kodesSeulex, 0);

    addToRunTimeSelectionTable(ODESolver, kodesSeulex, dictionary);

    const scalar
        kodesSeulex::stepFactor1_ = 0.6,
        kodesSeulex::stepFactor2_ = 0.93,
        kodesSeulex::stepFactor3_ = 0.1,
        kodesSeulex::stepFactor4_ = 4,
        kodesSeulex::stepFactor5_ = 0.5,
        kodesSeulex::kFactor1_ = 0.7,
        kodesSeulex::kFactor2_ = 0.9;
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::kodesSeulex::kodesSeulex(const ODESystem& ode, const dictionary& dict)
:
    ODESolver(ode, dict),
    ...
    logStepCount_(0)
{
    ...
    Info<< "kodesSeulex: n = " << n_
        << ", absTol[0] = " << absTol_[0]
        << ", relTol[0] = " << relTol_[0] << endl;
}


// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //

bool Foam::kodesSeulex::seul
(
    const scalar x0,
...
) const
{
    ...
    // --- KODES diagnostic: same placement/format as Seulex.cu's
    // "workIndex == 0 derivatives" block (right after derivatives(), before
    // the LU back-substitution that turns dy_ into the implicit-Euler update).
    {
        Info<< "kodesSeulex derivatives (k=" << k << ", x=" << xnew << ")" << nl;
        forAll(dy_, i)
        {
            Info<< setprecision(16) << dy_[i] << ' ';
        }
        Info<< nl;
    }
...
void Foam::kodesSeulex::extrapolate
(
    const label k,
...
{
    // --- KODES diagnostic: same placement/format as Seulex.cu's
    // "workIndex == 0" state-vector + "dx=... x=..." block, printed at the
    // top of every outer step, before this step is attempted.
    {
        Info<< "kodesSeulex step " << logStepCount_++ << nl
            << "y =";
        forAll(y, i)
        {
            Info<< ' ' << setprecision(10) << y[i];
        }
        Info<< nl
            << "dx=" << setprecision(16) << step.dxTry
            << " x=" << setprecision(16) << x << endl;
    }
